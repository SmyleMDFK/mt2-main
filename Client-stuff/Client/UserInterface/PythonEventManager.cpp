...
void CPythonEventManager::SetVisibleLineCount(int32_t index, int32_t linecount) {
	if (!CheckEventSetIndex(index)) {
		return;
	}

	TEventSet * pEventSet = m_EventSetVector[index];
	if (pEventSet) {
		pEventSet->iVisibleLineCount = linecount;
	}
}

void CPythonEventManager::SetFontColor(int32_t index, float r, float g, float b, float a) {
	if (!CheckEventSetIndex(index)) {
		return;
	}
	...
	pEventSet->CurrentColor.a = a;
}

int32_t CPythonEventManager::GetLineHeight(int32_t index) {
	if (!CheckEventSetIndex(index)) {
		return 0;
	}
...
void CPythonEventManager::SetYPosition(int32_t index, int32_t y) {
	if (!CheckEventSetIndex(index)) {
		return;
	}
...
int32_t CPythonEventManager::GetProcessedLineCount(int32_t index) {
	if (!CheckEventSetIndex(index)) {
		return 0;
	}
...
void CPythonEventManager::AllProcessEventSet(int32_t index) {
	if (!CheckEventSetIndex(index)) {
		return;
	}
