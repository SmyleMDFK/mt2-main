...
class CharacterDetailsUI(ui.ScriptWindow):
	...
	def __Initialize(self):
		...
		##TODONOWself.InfoList.append( [ localeInfo.DETAILS_77, localeInfo.DETAILS_TOOLTIP_77, item.GetApplyPoint( item.APPLY_RESIST_MAGIC_REDUCTION ) ] )
			...
			if idx < len(self.InfoList) :
				...
				if type == self.CATEGORY_STARTLINE:
					...
					if i+self.LabelLineCount < self.UI_MAX_COUNT:
						self.labelTextList[i+self.LabelLineCount].SetShowToolTipEvent( ui.__mem_func__(self.__ButtonOverIn), self.LabelLineCount + self.startline_endlinecount )
						self.labelTextList[i+self.LabelLineCount].SetHideToolTipEvent( ui.__mem_func__(self.__ButtonOverOut), self.LabelLineCount + self.startline_endlinecount )
