...
class MyCharacters :
	...
	def LoadCharacterData(self) :
		...
		for i in xrange(net.CHARACTER_SLOT_COUNT_MAX) :
			...
			#TODO acce = net.GetAccountCharacterSlotDataInteger(i, net.ACCOUNT_CHARACTER_SLOT_ACCE)
			acce = 0
	...
	def MakeCharacter(self, slot, name, race, form, hair, acce):
		...
		chr.SetArmor(form)
		chr.SetHair(hair)
		#TODOchr.SetAcce(acce)
	
		chr.SetMotionMode(chr.MOTION_MODE_GENERAL)
		chr.SetLoopMotion(chr.MOTION_INTRO_WAIT)
		
		## 수인족만, 스케일 조금 축소.
		#TODOif chr.RaceToJob(race) == JOB_WOLFMAN:
		#TODO	chr.SetScale(0.95,0.95,0.95)
...
class SelectCharacterWindow(ui.Window) :
	...
	def Open(self):
		...
		##TODO
		getChild("select_thinboard_page").Hide()
		
		self.statImg = []
		self.statImg.append(getChild("hth_img"))
		self.statImg[0].SetEvent(ui.__mem_func__(self.OverInToolTipETC), "mouse_over_in", uiScriptLocale.CREATE_HP)
		self.statImg[0].SetEvent(ui.__mem_func__(self.OverOutToolTip), "mouse_over_out", 0)
		self.statImg.append(getChild("int_img"))
		self.statImg[1].SetEvent(ui.__mem_func__(self.OverInToolTipETC), "mouse_over_in", uiScriptLocale.CREATE_SP)
		self.statImg[1].SetEvent(ui.__mem_func__(self.OverOutToolTip), "mouse_over_out", 0)
		self.statImg.append(getChild("str_img"))
		self.statImg[2].SetEvent(ui.__mem_func__(self.OverInToolTipETC), "mouse_over_in", uiScriptLocale.CREATE_ATT_GRADE)
		self.statImg[2].SetEvent(ui.__mem_func__(self.OverOutToolTip), "mouse_over_out", 0)
		self.statImg.append(getChild("dex_img"))
		self.statImg[3].SetEvent(ui.__mem_func__(self.OverInToolTipETC), "mouse_over_in", uiScriptLocale.CREATE_DEX_GRADE)
		self.statImg[3].SetEvent(ui.__mem_func__(self.OverOutToolTip), "mouse_over_out", 0)
		
		##TODO
		self.statSungmaImg = []
		self.statSungmaImg.append(getChild("sungma_str_img"))
		self.statSungmaImg[0].SetEvent(ui.__mem_func__(self.OverInToolTipETC), "mouse_over_in", localeInfo.STAT_TOOLTIP_SUNGMA_STR)
		self.statSungmaImg[0].SetEvent(ui.__mem_func__(self.OverOutToolTip), "mouse_over_out", 0)
		self.statSungmaImg.append(getChild("sungma_hp_img"))
		self.statSungmaImg[1].SetEvent(ui.__mem_func__(self.OverInToolTipETC), "mouse_over_in", localeInfo.STAT_TOOLTIP_SUNGMA_HP)
		self.statSungmaImg[1].SetEvent(ui.__mem_func__(self.OverOutToolTip), "mouse_over_out", 0)
		self.statSungmaImg.append(getChild("sungma_move_img"))
		self.statSungmaImg[2].SetEvent(ui.__mem_func__(self.OverInToolTipETC), "mouse_over_in", localeInfo.STAT_TOOLTIP_SUNGMA_MOVE)
		self.statSungmaImg[2].SetEvent(ui.__mem_func__(self.OverOutToolTip), "mouse_over_out", 0)
		self.statSungmaImg.append(getChild("sungma_immune_img"))
		self.statSungmaImg[3].SetEvent(ui.__mem_func__(self.OverInToolTipETC), "mouse_over_in", localeInfo.STAT_TOOLTIP_SUNGMA_IMMUNE)
		self.statSungmaImg[3].SetEvent(ui.__mem_func__(self.OverOutToolTip), "mouse_over_out", 0)
		
		self.statOrigin = getChild("origin_stat_button")
		self.statOrigin.ShowToolTip = lambda arg = uiScriptLocale.SELECT_ORIGIN_STAT : self.OverInToolTipETC(arg)
		self.statOrigin.HideToolTip = lambda : self.OverOutToolTip()
		self.statOrigin.SetEvent(ui.__mem_func__(self.SelectStat), 0)
		self.statSungma = getChild("sungma_stat_button")
		self.statSungma.ShowToolTip = lambda arg = uiScriptLocale.SELECT_SUNGMA_STAT : self.OverInToolTipETC(arg)
		self.statSungma.HideToolTip = lambda : self.OverOutToolTip()
		self.statSungma.SetEvent(ui.__mem_func__(self.SelectStat), 1)
	...
	def SelectStat(self, arg):
		if arg == 0:
			for item in self.statSungmaImg:
				item.Hide()
			
			for item in self.statImg:
				item.Show()
		else:
			for item in self.statImg:
				item.Hide()
	...
	def Close(self):
		...
		self.statValue = None#[]
		self.statImg = None#[]
		self.statSungmaImg = None#[]
		self.statOrigin = None
		self.statSungma = None
		self.GaugeList = None#[]
		self.textBoard = None
		self.btnPrev = None
		self.btnNext = None
	...
	def ResetStat(self):
		## TODO
		if self.statSungmaImg[0].IsShow():
			myStatPoint = [0, 0, 0, 0]
		else:
	...
	def OverInToolTipETC(self, arg, arg2 = 0):
		if arg2 != 0:
			arglen = len(str(arg2))
		else:
			arglen = len(str(arg))
		...
		if arg2 != 0:
			self.toolTip.AppendTextLine(arg2, 0xffffff00)
		else:
			self.toolTip.AppendTextLine(arg, 0xffffff00)
		self.toolTip.Show()
		self.ShowToolTip = True

	def OverOutToolTip(self, arg1 = 0, arg2 = 0):
		self.toolTip.Hide()
		self.ShowToolTip = False
