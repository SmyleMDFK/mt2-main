		...
		for (int j = 0; j < PLAYER_PER_ACCOUNT; ++j)
		{
			if (pkTab->players[j].dwID == player_id)
			{
				...
				if (pt)
				{
					...
					std::string query = "SELECT UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(last_play) FROM player WHERE id=";
					query += std::to_string(player_id);
					std::auto_ptr<SQLMsg> msg(CDBManager::instance().DirectQuery(query.c_str(), SQL_PLAYER));
					uint32_t last_playtime = 0;
					if (msg->Get()->uiNumRows)
					{
						if (msg->Get()->pSQLResult)
						{
							MYSQL_ROW row = mysql_fetch_row(msg->Get()->pSQLResult);
							str_to_number(last_playtime, row[0]);
						}
					}
					pkTab->players[j].last_playtime = last_playtime;
				}
				else
				{
					if (!row[col++])
...
*/
					...
					str_to_number(pkTab->players[j].last_playtime, row[col]);
				}
...
void CClientManager::RESULT_LOGIN(CPeer * peer, SQLMsg * msg)
{
	...
	if (info->account_index == 0)
	{
		...
		else
		{
			...
			if (g_stLocale == "gb2312")
			{
				snprintf(queryStr, sizeof(queryStr), "SELECT id, name, job, level, alignment, st, ht, dx, iq, part_main, part_hair, x, y, skill_group, change_name, UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(last_play) FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
			}
			else
			{
				snprintf(queryStr, sizeof(queryStr), "SELECT id, name, job, level, playtime, st, ht, dx, iq, part_main, part_hair, x, y, skill_group, change_name, UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(last_play) FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
			}
